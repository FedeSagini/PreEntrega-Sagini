...
import pcArmadas from "../data/pcArmadas";
...
function ItemListContainer(props) {
    ...
    useEffect(() => {
        if (parseInt(categoryId) === 1) {
            productService.getAll(pcArmadas).then(resp => setCategoryArray(resp));
        } else if (parseInt(categoryId) === 2) {
            productService.getAll(notebooks).then(resp => setCategoryArray(resp));
        } else {
